...
**Goal of this stage:** encode trust in the models with automated checks, so
we can run this reliably locally now and in scheduled runs later.
...
And you should be able to build the funnel mart and its upstream dependencies:
...
- The required schema tests exist for `stg_website_events`, `stg_flower_orders`, and `fct_customer_funnel`.
- `dbt test` passes for the required tests.
- At least one singular business logic test exists under `tests/`.
- If optional sanity checks are too strict, consider altering their `severity` to `warn` so that you are still able to move to the next stage.
